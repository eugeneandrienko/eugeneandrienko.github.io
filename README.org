...
#+begin_src bash :shebang "#!/usr/bin/env bash"
  ...
  for article in $(find "$ARTICLES" -type f -regextype grep -regex '.\{1,\}/article-[a-z]\{2\}\.org'); do
...
#+begin_src bash
  sed -ri "2i #+LANG: $LANGUAGE" "$FILEPATH"/"$FILENAME".org
#+end_src
...
formatter - put all articles written in =$LANGAUGE= to =_posts/$LANGUAGE= catalog.
#+begin_src bash
  ...
  find "$FILEPATH" -type f -regextype grep -not -regex '.\{1,\}/article-[a-z]\{2\}\.org' -exec cp {} "$STATIC" \;
done
#+end_src
...
#+begin_src makefile :tangle Makefile
  ...
  clean:
          ...
          rm -f _posts/*.md && \
          rm -f assets/static/*{.png,.jpg,.jpeg,.JPG,.svg,.html}
#+end_src
